...
(define (helper a b)
  (if (and (pz? a) (not (pz? b))) (helper b a)
      (if (pz? b)
          a
          (helper (pred a) (pred b))
          )
      )
  )
...
(define (p-prime? p)
  "TODO: Implement"
  (helper2 p (succ (succ z)))
  )
(define (helper2 p q)
  (cond
    [(or (p> (succ (succ z)) p) (p= (succ (succ z)) p)) (cond
                                                          [(p= (succ (succ z)) p) #t]
                                                          [else #f]
                                                          )]
    [(pz? (p% p q)) #f]
    [(p> (p* q q) p) #t]
    [else (helper2 p (p+ q (succ z)))]
    )
  )